...
char    *version     = "2.00";                /* program version          */
char    *editlvl     = "12";                  /* edit level               */
char    *moddate     = "15-Apr-2000";         /* modification date        */
...
int main(int argc, char *argv[])
{
   ...
   else if (!access("/usr/local/etc/webalizer.conf",F_OK))
      get_config("/usr/local/etc/webalizer.conf");
   ...
   while (*cp1)
   {
      if (*cp1=='%')                            /* Found an escape?        */
      {
         ...
         if (isxdigit(*cp1))                    /* ensure a hex digit      */
         {
            ...
            if ((*cp2<32)||(*cp2==127)) *cp2='_'; /* underscore if bad     */
            if (*cp1) cp2++; cp1++;
         }
         else *cp2++='%';
      }
